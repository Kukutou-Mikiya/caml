"""
    Read four files with format (HADM_ID, CODE, INDEX),这里的index指的是在一个文本中提供了最大激活值的n-gram的在所有n-gram中的index。szs是window的size。
...
def main():
    ...
    with open('%s/qualitative_eval_full.md' % (MIMIC_3_DIR), 'w', newline='') as of:
        with open('%s/qualitative_eval_full_key.md' % (MIMIC_3_DIR), 'w', newline='') as kf:
            code_counts = Counter()
