...
def test(model, Y, epoch, data_path, fold, gpu, version, code_inds, dicts, samples, model_dir, testing):
    ...
    for batch_idx, tup in tqdm(enumerate(gen)):
        ...
        #修正一个pytorch版本导致的issue:适用于pytorch0.4.0以后版本
        ...
        #修正一个pytorch版本导致的issue:适用于pytorch0.4.0以后版本 用with torch.no_grad()代替volatile=True
        with torch.no_grad():
            output, loss, alpha = model(data, target, desc_data=desc_data, get_attention=get_attn)
        #修正一个pytorch版本导致的issue: nn.functional.sigmoid is deprecated. Use torch.sigmoid instead.
